...
async function handleForward(req: Request, method: string) {
  try {
    ...
    const backendRes = await fetch(`${BACKEND_INTERNAL_URL}/api/admin/students/division${url.search}`, {
      method,
      headers,
    ...
    return NextResponse.json(
      { success: false, message: 'Failed to communicate with division assignment service' },
      { status: 502 }
    );
  }
}
