...
// ─── Timetable Page ───────────────────────────────────────────────────────────
const schedule = [
  { time: '09:00 AM - 10:00 AM', subject: 'Data Structures', faculty: 'Dr. Rakesh', room: 'Lab 1', status: 'Completed' },
  { time: '10:00 AM - 11:00 AM', subject: 'Algorithms', faculty: 'Prof. Sunita', room: 'Room 302', status: 'Ongoing' },
  { time: '11:15 AM - 12:15 PM', subject: 'Database', faculty: 'Dr. Vivek', room: 'Room 304', status: 'Upcoming' },
]
      ...
      <AdminContent>
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-2 mb-4">
             <select className="h-9 rounded-md border border-input bg-background px-3 text-sm outline-none focus:ring-1 focus:ring-ring min-w-[150px]">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-border">
                  {schedule.map((s, i) => (
                    <tr key={i} className="hover:bg-muted/50 transition-colors">
                      <td className="px-6 py-4 font-medium text-foreground">{s.time}</td>
                      <td className="px-6 py-4">{s.subject}</td>
                      <td className="px-6 py-4 text-muted-foreground">{s.faculty}</td>
                      <td className="px-6 py-4 text-muted-foreground">{s.room}</td>
                      <td className="px-6 py-4">
                        <span className={`text-xs font-medium ${s.status === 'Ongoing' ? 'text-blue-600' : s.status === 'Upcoming' ? 'text-amber-600' : 'text-green-600'}`}>
                          {s.status}
                        </span>
                      </td>
                    </tr>
